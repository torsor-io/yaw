...
def bell_ket(i, j, observable=None, algebra=None):
    """Construct Bell state ket |Φᵢⱼ⟩ or |Ψᵢⱼ⟩.
    
    The four Bell states in Z basis:
    - |Φ⁺⟩ = (|00⟩ + |11⟩)/√2  → bell_ket(0, 0) 
    - |Φ⁻⟩ = (|00⟩ - |11⟩)/√2  → bell_ket(0, 1)
    - |Ψ⁺⟩ = (|01⟩ + |10⟩)/√2  → bell_ket(1, 0)
    - |Ψ⁻⟩ = (|01⟩ - |10⟩)/√2  → bell_ket(1, 1)
    
    Args:
        i: First index (0 or 1)
        j: Second index (0 or 1)
        observable: Observable to use (default: Z)
        algebra: Algebra context (uses default if not provided)
        
    Returns:
        SumState representing the Bell state ket
    """
    ...
    if observable is None:
        # Default to Z basis
        if algebra is None:
            algebra = Algebra(gens=['X', 'Z'], rels=['herm', 'unit', 'anti', 'pow(2)'])
        observable = algebra.Z
    
    # Construct basis states
    if i == 0 and j == 0:  # Φ⁺
        ket1 = char(observable, 0) @ char(observable, 0)
        ket2 = char(observable, 1) @ char(observable, 1)
        return SumState([ket1, ket2]) / sqrt(2)
    elif i == 0 and j == 1:  # Φ⁻
        ket1 = char(observable, 0) @ char(observable, 0)
        ket2 = char(observable, 1) @ char(observable, 1)
        # Create ScaledState for -ket2
        from yaw_prototype import ScaledState
        return SumState([ket1, ScaledState(ket2, -1)]) / sqrt(2)
    elif i == 1 and j == 0:  # Ψ⁺
        ket1 = char(observable, 0) @ char(observable, 1)
        ket2 = char(observable, 1) @ char(observable, 0)
        return SumState([ket1, ket2]) / sqrt(2)
    elif i == 1 and j == 1:  # Ψ⁻
        ket1 = char(observable, 0) @ char(observable, 1)
        ket2 = char(observable, 1) @ char(observable, 0)
        ...
        raise ValueError(f"Invalid indices ({i}, {j}). Must be (0,0), (0,1), (1,0), or (1,1)")


def bell_bra(i, j, observable=None, algebra=None):
    """Construct Bell state bra (functional) ⟨Φᵢⱼ| or ⟨Ψᵢⱼ|.
    
    The four Bell state functionals (using X⊗X symmetry):
    - ⟨Φ⁺| = (⟨00| + X⊗X·⟨00| + ⟨11| + X⊗X·⟨11|)/2  → bell_bra(0, 0)
    - ⟨Φ⁻| = (⟨00| + X⊗X·⟨00| - ⟨11| - X⊗X·⟨11|)/2  → bell_bra(0, 1)
    - ⟨Ψ⁺| = (⟨01| + X⊗X·⟨01| + ⟨10| + X⊗X·⟨10|)/2  → bell_bra(1, 0)
    - ⟨Ψ⁻| = (⟨01| + X⊗X·⟨01| - ⟨10| - X⊗X·⟨10|)/2  → bell_bra(1, 1)
    ...
    Returns:
        SumState representing the Bell state functional
    """
    if observable is None:
        if algebra is None:
            algebra = Algebra(gens=['X', 'Z'], rels=['herm', 'unit', 'anti', 'pow(2)'])
        ...
        if not hasattr(observable, 'algebra'):
            raise ValueError("Observable must have an algebra attribute")
    ...
    # Construct basis states
    if i == 0 and j == 0:  # ⟨Φ⁺|
        psi1 = char(observable, 0) @ char(observable, 0)
        psi2 = char(observable, 1) @ char(observable, 1)
        return SumState([psi1, X_X % psi1, psi2, X_X % psi2]) / 2
    elif i == 0 and j == 1:  # ⟨Φ⁻|
        from yaw_prototype import ScaledState
        psi1 = char(observable, 0) @ char(observable, 0)
        psi2 = char(observable, 1) @ char(observable, 1)
        return SumState([psi1, X_X % psi1, 
                        ScaledState(psi2, -1), ScaledState(X_X % psi2, -1)]) / 2
    elif i == 1 and j == 0:  # ⟨Ψ⁺|
        psi1 = char(observable, 0) @ char(observable, 1)
        psi2 = char(observable, 1) @ char(observable, 0)
        return SumState([psi1, X_X % psi1, psi2, X_X % psi2]) / 2
    elif i == 1 and j == 1:  # ⟨Ψ⁻|
        from yaw_prototype import ScaledState
        psi1 = char(observable, 0) @ char(observable, 1)
...
class BellKetCallable:
    """Callable object for Bell state kets: Phi(i, j) = |Φᵢⱼ⟩"""
    def __init__(self, observable=None, algebra=None):
        self.observable = observable
        self.algebra = algebra
    
    def __call__(self, i, j):
        return bell_ket(i, j, self.observable, self.algebra)
    
    def __repr__(self):
        return "BellKet(i, j) -> |Φᵢⱼ⟩ or |Ψᵢⱼ⟩"


class BellBraCallable:
    """Callable object for Bell state bras: phi(i, j) = ⟨Φᵢⱼ|"""
    def __init__(self, observable=None, algebra=None):
        self.observable = observable
        self.algebra = algebra
    
    def __call__(self, i, j):
        return bell_bra(i, j, self.observable, self.algebra)
    
    def __repr__(self):
        return "BellBra(i, j) -> ⟨Φᵢⱼ| or ⟨Ψᵢⱼ|"
